...
public class EncheresSecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(auth -> {
			// les trois modèles courants à définir (rappel : 'hasAnyRole' pour plusieurs et
...
//				.requestMatchers(" ").hasRole("ADMINISTRATEUR");
			...
			auth.requestMatchers(HttpMethod.GET, "/user/**")
				...
				.authenticated();
			auth.requestMatchers(HttpMethod.GET, "/Creer-Article")
				.authenticated()
				.requestMatchers(HttpMethod.POST, "/Creer-Article")
				.authenticated();
			auth.requestMatchers(HttpMethod.GET, "/articles/articleDetail/**")
				.permitAll()
...
//			auth.anyRequest().denyAll();//accès refusé pour toutes les autres URL.
		...
		// on demande à spring security de charger son formulaire de connexion à défaut
		// d'en avoir créé un nous-même
