...
public class EncheresSecurityConfig {
	...
	UserDetailsManager userManager(DataSource dataSource) {
		...
		jdbcUserManager.setUsersByUsernameQuery("SELECT pseudo, password, 1 FROM UTILISATEUR WHERE pseudo = ?");
		// 2. Requête recherchant les utilisateurs par leurs rôles, en utilisant un paramètre positionnel (?)
		jdbcUserManager.setAuthoritiesByUsernameQuery("SELECT u.pseudo, r.is_admin FROM UTILISATEUR u INNER JOIN ROLES r ON u.administrateur = r.is_admin WHERE pseudo = ?");
